...
def main():
    if len(sys.argv) < 2:
        print("Usage: linkedin_dm_result.py <exec_file> [status]")
    ...
    result = {
        ...
        "error": "Rate limited by LinkedIn" if status == "rate_limited" else ("Toast error detected" if status == "failed" else None),
        "duration_ms": 0
    }
