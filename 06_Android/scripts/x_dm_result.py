#!/usr/bin/env python3
"""
X/Twitter DM Result Handler
Called by Tasker AFTER the 3-layer UI chain completes.
Reads exec file, checks for rate limits via toast, writes final result.
"""
...
def main():
    if len(sys.argv) < 2:
        print("Usage: x_dm_result.py <exec_file> [status]")
        print("  status: sent | failed | rate_limited (optional, auto-detected if omitted)")
        sys.exit(1)
